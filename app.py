...
#         )
#         # Test if the OpenAI connection works by sending a simple request
#         openai_client.chat.completions.create(model=deployment, messages=[{"role": "system", "content": "test"}])
...
#             # List containers to verify the connection
#             containers = blob_service_client.list_containers()
            
#             st.success("Connection successful! Containers in the storage account:")
#             for container in containers:
#                 print(f"- {container['name']}")
#         except HttpResponseError as e:
#             st.error(f"Azure Blob Storage connection failed with error: {e}")
        

#         #Check Azure Cognitive Search connection
#         #search_url = f"{search_endpoint}/indexes/{search_index}/docs"
#         search_url = f"{search_endpoint}/indexes/{search_index}/docs?api-version=2023-07-01-Preview"
#         headers = {'Content-Type': 'application/json', 'api-key': search_api_key}
#         response = requests.get(search_url, headers=headers)
#         #print("Line 59", response)
#         if response.status_code == 200:
#             st.success("Successfully connected to Azure Cognitive Search")
#         else:
#             st.error(f"Failed to connect to Azure Cognitive Search: {response.status_code}")

#     except Exception as e:
#         st.error(f"Azure connection error: {str(e)}")

# check_azure_connections()
...
if 'avatar_user' not in st.session_state:
    st.session_state['avatar_user'] = open_avatare_image('imgs/avatar_user.png')
if 'avatar_ai' not in st.session_state:
    st.session_state['avatar_ai'] = open_avatare_image('imgs/avatar_ai.png')
...
with st.sidebar:
    ...
    if on:
        ...
        # with c2:
            # st.write('#### Download Your Conversatioon')
        ...
        with c2:
            download_conversation_button = st.download_button(
                ...
                data="\n".join(st.session_state["generated"]),  # Convert list to string with newlines between messages
                file_name="conversation.txt",
                mime="text/plain",  # Use the appropriate MIME type for text files
            )
                ...
                with tab:
                    ...
                    for img in ref["imgurls"]:
                        ...
                        try:
                            ...
                            pass
                        base_url = re.sub(r'\?.*$', '', img)  # Removes everything after ?
                        print(base_url)

                        st.image(base_url, use_container_width ="auto", caption=caption)
...
st.image("imgs/UltanLogo1.gif")

st.title("Ultan RAG Chatbot")
